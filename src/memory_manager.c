...
block_header_t* mem_find_free_block_best_fit(size_t size)
{
    ...
    while (current != NULL)
    {
        if (current->state == BLOCK_FREE && current->size >= size && current->size < best_size)
        {
            best_block = current;
...
static block_header_t* get_block_header(void* ptr)
{
    ...
    if ((uint8_t*)block < g_heap_buffer || (uint8_t*)block >= g_heap_buffer + HEAP_SIZE)
    {
        return NULL;
    }
...
int mem_validate_heap(void)
{
    ...
    while (current_addr < g_heap_buffer + HEAP_SIZE)
    {
        ...
        // Validar tamaño
        if (block->size == 0 || current_addr + sizeof(block_header_t) + block->size > g_heap_buffer + HEAP_SIZE)
        {
    ...
    if (calculated_free != g_heap_control.free_memory || calculated_used != g_heap_control.used_memory)
    {
        fprintf(stderr, "Statistics mismatch: calculated free=%zu used=%zu, stored free=%zu used=%zu\n",
                calculated_free, calculated_used, g_heap_control.free_memory, g_heap_control.used_memory);
        return -1;
    }
