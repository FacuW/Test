...
int sandbox_init(sandbox_config_t* config)
{
    ...
    if (config) {
        ...
        strncpy(g_sandbox_config.plugins_dir, SANDBOX_PLUGINS_DIR, 
                sizeof(g_sandbox_config.plugins_dir) - 1);
        strncpy(g_sandbox_config.work_dir, SANDBOX_WORK_DIR, 
                sizeof(g_sandbox_config.work_dir) - 1);
        strncpy(g_sandbox_config.log_file, SANDBOX_LOG_FILE, 
                sizeof(g_sandbox_config.log_file) - 1);
...
int sandbox_validate_plugin_path(const char* plugin_path)
{
    ...
    if (strncmp(resolved_path, g_sandbox_config.plugins_dir, 
                strlen(g_sandbox_config.plugins_dir)) != 0) {
...
int sandbox_validate_input_path(const char* input_path)
{
    ...
    if (strncmp(resolved_path, g_sandbox_config.work_dir, 
                strlen(g_sandbox_config.work_dir)) != 0) {
...
int sandbox_log_error(const char* plugin_name, sandbox_result_t error, const char* message)
{
    ...
    fprintf(log_file, "[%s] ERROR plugin=%s error=%s message=\"%s\"\n",
            timestamp, plugin_name ? plugin_name : "unknown",
            sandbox_result_to_string(error),
            message ? message : "");
 ...
 */
int sandbox_execute_plugin(const char* plugin_name, 
                          ...
                          sandbox_execution_t* execution)
{
    ...
    snprintf(plugin_path, sizeof(plugin_path), "%s%s", 
             g_sandbox_config.plugins_dir, plugin_name);
    ...
    if (pid == 0) {
        ...
        analizar_datos_func_t analizar_datos = 
            (analizar_datos_func_t)sandbox_dlsym(handle, "analizar_datos");
        ...
        // Leer stdout
        ssize_t stdout_read = read(stdout_pipe[0], execution->stdout_data, 
                                 sizeof(execution->stdout_data) - 1);
        if (stdout_read > 0) {
        ...
        // Leer stderr
        ssize_t stderr_read = read(stderr_pipe[0], execution->stderr_data, 
                                 sizeof(execution->stderr_data) - 1);
        if (stderr_read > 0) {
        ...
        if (WIFEXITED(status)) {
            ...
            execution->result = (execution->exit_code == 0) ? 
                               SANDBOX_SUCCESS : SANDBOX_ERROR_EXECUTION_FAILED;
...
int sandbox_list_plugins(plugin_info_t plugins[], int max_plugins)
{
    ...
    while ((entry = readdir(dir)) != NULL && plugin_count < max_plugins) {
        ...
        snprintf(full_path, sizeof(full_path), "%s%s", 
                g_sandbox_config.plugins_dir, entry->d_name);
        
        struct stat st;
        if (stat(full_path, &st) == 0 && S_ISREG(st.st_mode)) {
            strncpy(plugins[plugin_count].name, entry->d_name, 
                   sizeof(plugins[plugin_count].name) - 1);
            strncpy(plugins[plugin_count].path, full_path, 
                   sizeof(plugins[plugin_count].path) - 1);
            
            plugins[plugin_count].last_modified = st.st_mtime;
            plugins[plugin_count].file_size = st.st_size;
            plugins[plugin_count].is_valid = 
                (sandbox_validate_plugin_path(full_path) == 0) ? 1 : 0;
            
            strncpy(plugins[plugin_count].description, "C++ Plugin", 
                   sizeof(plugins[plugin_count].description) - 1);
...
const char* sandbox_result_to_string(sandbox_result_t result)
{
    switch (result) {
        ...
        case SANDBOX_ERROR_INVALID_PATH: return "INVALID_PATH";
        case SANDBOX_ERROR_PLUGIN_NOT_FOUND: return "PLUGIN_NOT_FOUND";
        case SANDBOX_ERROR_LOAD_FAILED: return "LOAD_FAILED";
        case SANDBOX_ERROR_SYMBOL_NOT_FOUND: return "SYMBOL_NOT_FOUND";
        case SANDBOX_ERROR_EXECUTION_FAILED: return "EXECUTION_FAILED";
        case SANDBOX_ERROR_TIMEOUT: return "TIMEOUT";
        case SANDBOX_ERROR_RESOURCE_LIMIT: return "RESOURCE_LIMIT";
        case SANDBOX_ERROR_PERMISSION_DENIED: return "PERMISSION_DENIED";
        case SANDBOX_ERROR_SETUP_FAILED: return "SETUP_FAILED";
...
{
    switch (mode) {
        case SANDBOX_MODE_A: return "Mode A (Full isolation with chroot)";
        case SANDBOX_MODE_B: return "Mode B (Basic isolation)";
...
{
    if (!g_sandbox_initialized) return -1;
...
{
    if (!config) config = &g_sandbox_config;
