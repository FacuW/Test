...
void storage_generate_filename(char* buffer, size_t size, time_t date)
{
    ...
    snprintf(buffer, size, "%s%s%04d%02d%02d%s",
             STORAGE_DIR, STORAGE_FILE_PREFIX,
...
static void init_storage_header(storage_header_t* header)
{
    ...
    header->header_crc = storage_calculate_crc32(header, 
                         sizeof(storage_header_t) - sizeof(header->header_crc) - sizeof(header->reserved));
}
...
int storage_validate_header(const storage_header_t* header)
{
    ...
    uint32_t calculated_crc = storage_calculate_crc32(header,
                             sizeof(storage_header_t) - sizeof(header->header_crc) - sizeof(header->reserved));
...
int storage_metrics_to_record(const system_metrics_t* metrics, storage_record_t* record)
{
    ...
    record->data_length = sizeof(storage_record_t) - offsetof(storage_record_t, cpu_usage) - 
    ...
    record->record_crc = storage_calculate_crc32(record, 
                        sizeof(storage_record_t) - sizeof(record->record_crc));
...
int storage_write_record(storage_handle_t* handle, const system_metrics_t* metrics)
{
    ...
    handle->header.header_crc = storage_calculate_crc32(&handle->header,
                               sizeof(storage_header_t) - sizeof(handle->header.header_crc) - 
...
int storage_list_files(char files[][STORAGE_MAX_PATH_LENGTH], int max_files)
{
    ...
    while ((entry = readdir(dir)) != NULL && file_count < max_files) {
        if (strstr(entry->d_name, STORAGE_FILE_PREFIX) == entry->d_name &&
            ...
            snprintf(files[file_count], STORAGE_MAX_PATH_LENGTH, "%s%s", 
...
void storage_dump_record(const storage_record_t* record)
{
    ...
    printf("Record: timestamp=%ld, cpu=%.2f%%, mem_used=%lu, load_1m=%.2f\n",
           record->timestamp, record->cpu_usage, record->memory_used, record->load_1m);
}
