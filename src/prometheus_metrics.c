...
int init_prometheus_metrics(void)
{
    ...
    prometheus_cpu_usage_percent = prom_gauge_new(
    ...
    prometheus_memory_total_bytes = prom_gauge_new(
        "monitoring_memory_total_bytes",
        "Total memory in bytes",
        0, NULL
    );
    
    prometheus_memory_free_bytes = prom_gauge_new(
        "monitoring_memory_free_bytes",
        "Free memory in bytes",
        0, NULL
    );
    
    prometheus_memory_used_bytes = prom_gauge_new(
        "monitoring_memory_used_bytes",
        "Used memory in bytes",
        0, NULL
    );
    
    // Crear métricas gauge para load average
    prometheus_load_average_1m = prom_gauge_new(
        "monitoring_load_average_1m",
        "Load average 1 minute",
        0, NULL
    );
    
    prometheus_load_average_5m = prom_gauge_new(
        "monitoring_load_average_5m",
        "Load average 5 minutes",
        0, NULL
    );
    
    prometheus_load_average_15m = prom_gauge_new(
        "monitoring_load_average_15m",
        "Load average 15 minutes",
        0, NULL
    );
    
    // Crear contador para métricas recolectadas
    prometheus_metrics_collected_total = prom_counter_new(
    ...
    if (!prometheus_cpu_usage_percent || !prometheus_memory_total_bytes ||
        !prometheus_memory_free_bytes || !prometheus_memory_used_bytes ||
        !prometheus_load_average_1m || !prometheus_load_average_5m ||
